---
title: "Visualise_IC50Normalisation_with_Clustering"
date: "2024-05-17"
...
#Select Data
#'Example for visualisation with anomalous data: examplekcv2
```{r Download Files}
...
input_file <- here::here("Validation/anomaly_detection/example1kcv2.xlsx") # CHANGE directory and name for yourself
...
```{r Create dataset}
#'Select the 3 maximum and 3 minimum values from the sample columns
...
#'The plot shows boxplots for the minimum and maximum three values for each sample. Boxplots in the pos and neg control columns represent each cluster. An hline() is created for the mean generated from the k-clustering function applied to the control values. From the positive control, the "Positive Control Cluster Mean" is the mean of the cluster that is closest to the average of the sample maximums (highest values on the max boxplots). From the negative control, the "Negative Control Cluster Mean" is the mean of the cluster that is closest to the average of the sample minimums (lowest values on the min boxplots).
```{r plot using the cluster means}
plot.new()
ggplot(df.visualize, aes(x=df.visualize$y, y=df.visualize$x, color=Label))+
...
#'Compared to simple means, it can be seen that the cluster mean is better for normalisation since it selects a mean that is closer to the maximum and minimum values. The hline for the positive control simple mean is much lower and not as close to the sample maximums as the selected cluster mean.
```{r plot with simple means}
plot.new()
ggplot(df.visualize, aes(x=df.visualize$y, y=df.visualize$x, color=Label))+
